...
void *save_data(void *arg)
{
	...
	ioSocket socketEFIS("10.10.0.1",5500);    // Cria um socket cliente nesta maquina, para se comunicar com a maquina especificada pelo IP 10.10.0.1
	...
	while(1) { // Salva os dados de todos os dispositivos (placa daq, ahrs e gps)
		...
		if ((local_end_save_data==SEND)||(local_end_save_data==SAVE_SEND)){
			
			if (ahrs_ok) socketEFIS.SendData(msg_ahrs.angle[0],msg_ahrs.angle[1], msg_ahrs.angle[2]);
		}		
